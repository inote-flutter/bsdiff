...
Uint8List bsdiff(List<int> olddata, List<int> newdata) {
  ...
  _qsufsort(idata, List<int>.filled(oldsize + 1,0), Uint8List.fromList(olddata));
  ...
  for (int scan = 0, len = 0, lastscan = 0, lastpos = 0, lastoffset = 0; scan < newsize; ) {
    ...
    for (int scsc = scan += len; scan < newsize; scan++) {
      len = _search(idata, Uint8List.fromList(olddata), Uint8List.fromList(newdata), scan, 0, oldsize, pos);
